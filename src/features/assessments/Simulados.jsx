...
export default function Simulados() {
  ...
  const loadSimulados = async () => {
    ...
    try {
      ...
      setErrorMessage(error.message || 'Não foi possível carregar os simulados.');
      setStatus('error');
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    if (Number(form.acertos) > Number(form.total)) {
      window.alert('O número de acertos não pode ser maior que o total de questões!');
      return;
    }
    ...
    try {
      if (editingId) {
        const updatedExam = await updateStudentMockExam(editingId, form);
        setSimulados((currentSimulados) =>
          currentSimulados.map((exam) =>
            exam.id === editingId ? updatedExam : exam,
          ),
        );
      } else {
        const createdExam = await createStudentMockExam(form);
        setSimulados((currentSimulados) => [...currentSimulados, createdExam]);
      }
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <Card className="border-red-100 bg-red-50/70">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-red-700">Não foi possível carregar os simulados</h3>
          ...
          {status === 'success' && simulados.length === 0 ? (
            <Card className="text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">
                Comece registrando seu primeiro resultado para acompanhar sua evolução.
              </p>
            </Card>
          ) : null}
        ...
        <div className="space-y-6">
          <Card>
            <h3 className="mb-4 text-lg font-bold text-slate-800">Visão rápida</h3>
            <div className="space-y-4">
              <div className="rounded-2xl bg-blue-50 p-4">
                <div className="flex items-center gap-2 text-blue-700">
                  <TrendingUp size={18} />
                  <span className="text-sm font-bold">Média geral</span>
                </div>
              ...
              <div className="rounded-2xl bg-slate-100 p-4">
                <div className="flex items-center gap-2 text-slate-600">
                  <Clock size={18} />
                  <span className="text-sm font-bold">Tempo médio</span>
                </div>
